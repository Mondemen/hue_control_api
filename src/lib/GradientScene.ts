...
export default class GradientScene
{
	protected parent: ResourceWithGradient;
	...
	static setData(gradient: GradientScene, data: GradientSet)
	{
		gradient.setData(data);
	}

	protected setData(data: GradientSet)
	{
		if (data.points)
		{
	...
	addColor(color: ColorValue)
	{
		...
		console.log("ADD COLOR GRADIANT");

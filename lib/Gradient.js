...
export default class Gradient
{
	...
	_setData(data)
	{
		data?.gradient?.points?.forEach((point, i) =>
		{
			...
			if (point?.color?.xy && (this._data[i].color?.xy?.x != point?.color?.xy?.x || this._data[i].color?.xy?.y != point?.color?.xy?.y))
			{
				...
				if (this._light)
				{
					this._parent.emit("action_gradient_color", this._light, i, new Color(point.color.xy));
					this._parent.emit("action_gradient_color_xy", this._light, i, point.color.xy);
				}
				else
				{
					this._parent.emit("gradient_color", i, new Color(point.color.xy));
					this._parent.emit("gradient_color_xy", i, point.color.xy);
				}
			}
		});
	}
	 ...
	 */
	getColorXY(index)
	{return (this._update[index]?.color?.xy ?? this._data[index]?.color?.xy);}
	 ...
	 */
	getColorsXY()
	{return ((this._update ?? this._data ?? []).map(point => point.color.xy))}

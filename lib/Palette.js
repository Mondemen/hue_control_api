...
export default class Palette
{
		...
		if (data?.palette?.color || data?.palette?.color_temperature || data?.palette?.dimming)
		{
			...
			data?.palette?.color_temperature?.forEach((color_temp, i) =>
			{
				...
				if (color_temp?.dimming?.brightness != undefined && this._data.palette.color_temperature[i]?.dimming?.brightness != color_temp.dimming.brightness)
				{
					...
					emit["color_temperature_brightness"] = true;
				}
	 ...
	 * @param {Color|string|{x: number, y: number, brightness: number}|{r: number, g: number, b: number}|{h: number, s: number, v: number}} color The color
	 ...
	 * 
	 * @param {Mired|Color|number|string|{x: number, y: number, brightness: number}|{r: number, g: number, b: number}|{h: number, s: number, v: number}} mired The color temperature
	 * @param {number} brightness The brightness
	 * @returns {Palette}
	 * @throws {ArgumentError}
	 */
	...
	{
		if (speed < 0 || speed > 1)
			throw new Error(`The speed must have between 0 and 1`);
		if (this._scene.isExists())
		{
			this._copyToUpdate();
